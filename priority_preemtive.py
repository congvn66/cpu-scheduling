    ...
    def run(self):
        ...
        while i < len(self.processes) or ready_queue:
            while i < len(self.processes) and self.processes[i].arrival <= time:
                p = self.processes[i]
                heapq.heappush(ready_queue, (p.priority, p.arrival, p.burst, p.id, p))
                i += 1
            ...
            priority, arrival, burst, pid, p = heapq.heappop(ready_queue)
            ...
            if remain > 0:
                heapq.heappush(ready_queue, (priority, arrival, remain, pid, p))
